...
#define DEBUG
	...
	#define WINDOWS
#elif __APPLE__
# self is unsupported for the moment
#elif __linux
	#define LINUX
#endif

#define Max_map_scale 16
# the maximum scale of the map view from the base window size, if the base
# window has a width of 1024 pixels, maximum zoom scale will encompass
# 1024*(10^16) meters in universe. Thats about 100 light years or so if I
# recall correctly, 1000? The scale might only go up to 10^15 or something
# in actual practice
...
std.string Convert_to_string(float num, precision)
std.string Convert_to_string(long int num)

namespace SI
{	std.string Get_formatted_value(float value, precision, unit)
	std.string Get_formatted_value(std.string prefix, value, precision, unit)
	
	std.string Get_formatted_value(long int value, unit)
	std.string Get_formatted_value(std.string prefix, int value, unit)
}
